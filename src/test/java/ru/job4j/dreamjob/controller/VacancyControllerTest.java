...
public class VacancyControllerTest {
    ...
    @Test
    public void whenGetVacancyByIdThenGetThatVacancyAndRedirectToVacancyPage() {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var vacancy2 = new Vacancy(2, "test2", "desc2", now(), false, 3, 4);
        when(vacancyService.findById(anyInt())).thenReturn(Optional.of(vacancy1));
    ...
    @Test
    public void whenSomeVacancyUpdateThenGetUpdatedVacancyAndRedirectToVacanciesPage() throws Exception {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        ...
        when(vacancyService.update(vacancyArgumentCapture.capture(),
                fileDtoArgumentCapture.capture())).thenReturn(true);

        var model = new ConcurrentModel();
        var view = vacancyController.update(vacancy1, testFile, model);
        var updatedVacancy = vacancyArgumentCapture.getValue();
        var actualFileDto = fileDtoArgumentCapture.getValue();

        assertThat(view).isEqualTo("redirect:/vacancies");
        assertThat(vacancy1).isEqualTo(updatedVacancy);
        assertThat(fileDto).usingRecursiveComparison().isEqualTo(actualFileDto);
    }

    @Test
    public void whenDeleteVacancyByIdThenRedirectToVacanciesPage() {
        var vacancy1 = new Vacancy(1, "test1", "desc1", now(), true, 1, 2);
